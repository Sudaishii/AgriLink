...
const run = async () => {
    ...
    const files = fs_1.default.readdirSync(uploadsDir).filter(f => /\.(jpg|jpeg|png|webp)$/i.test(f));
    if (files.length < 1) {
        console.log('No images in uploads folder to test with.');
        return;
    }
    console.log(`\n🔬 Testing CLIP local inference with: ${files[0]}\n`);
    const buf1 = fs_1.default.readFileSync(path_1.default.join(uploadsDir, files[0]));
    const emb1 = await (0, imageEmbeddingService_1.getImageEmbedding)(buf1, 'image/jpeg');
    ...
    if (files.length >= 2) {
        ...
        const buf2 = fs_1.default.readFileSync(path_1.default.join(uploadsDir, files[1]));
        const emb2 = await (0, imageEmbeddingService_1.getImageEmbedding)(buf2, 'image/jpeg');
        console.log(`✅ Embedding 2 — dimensions: ${emb2.length}`);
        const sim = (0, imageEmbeddingService_1.cosineSimilarity)(emb1, emb2);
    ...
    const selfSim = (0, imageEmbeddingService_1.cosineSimilarity)(emb1, emb1);
