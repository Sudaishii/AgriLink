...
async function setupDatabase() {
    try {
        ...
        const [categories] = await database_1.db.execute('SELECT COUNT(*) as count FROM product_category');
        if (categories[0].count === 0) {
            ...
            for (const [oldName, newName] of renames) {
                await database_1.db.execute('UPDATE product_category SET cat_name = ? WHERE cat_name = ?', [newName, oldName]).catch(() => { }); // ignore if duplicate
            }
            // Ensure all 7 categories exist
            const newCats = ['Rice & Corn', 'Vegetables', 'Leafy Greens', 'Root Crops', 'Fruits', 'Herbs & Spices', 'Beans & Nuts'];
            await database_1.db.execute('UPDATE product_category SET cat_name = ? WHERE cat_name = ?', ['Rice & Corn', 'Legumes']).catch(() => { });
            for (const cat of newCats) {
                await database_1.db.execute('INSERT IGNORE INTO product_category (cat_name) VALUES (?)', [cat]);
            }
        ...
        await database_1.db.execute('ALTER TABLE users_table ADD COLUMN IF NOT EXISTS phone VARCHAR(20) DEFAULT NULL');
        await database_1.db.execute('ALTER TABLE users_table ADD COLUMN IF NOT EXISTS address TEXT DEFAULT NULL');
        await database_1.db.execute('ALTER TABLE users_table ADD COLUMN IF NOT EXISTS city VARCHAR(100) DEFAULT NULL');
        await database_1.db.execute('ALTER TABLE users_table ADD COLUMN IF NOT EXISTS province VARCHAR(100) DEFAULT NULL');
        await database_1.db.execute('ALTER TABLE users_table ADD COLUMN IF NOT EXISTS zip_code VARCHAR(20) DEFAULT NULL');
        await database_1.db.execute('ALTER TABLE users_table ADD COLUMN IF NOT EXISTS latitude DECIMAL(10,8) DEFAULT NULL');
        await database_1.db.execute('ALTER TABLE users_table ADD COLUMN IF NOT EXISTS longitude DECIMAL(11,8) DEFAULT NULL');
        await database_1.db.execute('ALTER TABLE users_table ADD COLUMN IF NOT EXISTS role ENUM("buyer","farmer","brgy_official","lgu_official","admin") DEFAULT "buyer"');
        await database_1.db.execute('ALTER TABLE users_table ADD COLUMN IF NOT EXISTS profile_image VARCHAR(255) DEFAULT NULL');
        await database_1.db.execute('ALTER TABLE users_table ADD COLUMN IF NOT EXISTS bio TEXT DEFAULT NULL');
        // Keep compatibility with legacy farm columns when farms_table is absent in older schemas.
        await database_1.db.execute('ALTER TABLE users_table ADD COLUMN IF NOT EXISTS farm_address TEXT DEFAULT NULL');
        await database_1.db.execute('ALTER TABLE users_table ADD COLUMN IF NOT EXISTS farm_city VARCHAR(100) DEFAULT NULL');
        await database_1.db.execute('ALTER TABLE users_table ADD COLUMN IF NOT EXISTS farm_province VARCHAR(100) DEFAULT NULL');
        await database_1.db.execute('ALTER TABLE users_table ADD COLUMN IF NOT EXISTS farm_zip_code VARCHAR(20) DEFAULT NULL');
        await database_1.db.execute('ALTER TABLE users_table ADD COLUMN IF NOT EXISTS farm_latitude DECIMAL(10,8) DEFAULT NULL');
        await database_1.db.execute('ALTER TABLE users_table ADD COLUMN IF NOT EXISTS farm_longitude DECIMAL(11,8) DEFAULT NULL');
        await database_1.db.execute('ALTER TABLE users_table ADD COLUMN IF NOT EXISTS farm_address_same_as_home TINYINT(1) DEFAULT 1');
