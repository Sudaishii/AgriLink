...
exports.createLog = exports.getAllLogs = exports.writeLog = exports.ensureSystemLogTable = void 0;
const database_1 = require("../database/database");
let ensuredLogTable = false;
// ─── Schema Ensure ─────────────────────────────────────────────────────────
const ensureSystemLogTable = async () => {
    if (ensuredLogTable)
        return;
    // Use the user-provided concise schema
    // Note: We DROP and RECREATE to strictly follow the "fix to only cater this" request
    try {
        const [rows] = await database_1.db.execute("SHOW TABLES LIKE 'system_logs'");
        if (rows.length > 0) {
            // Check if log_id is already INT to avoid unnecessary drops in dev
            const [cols] = await database_1.db.execute("DESCRIBE system_logs");
            const idCol = cols.find((c) => c.Field === 'log_id');
            if (idCol && idCol.Type.toLowerCase().includes('varchar')) {
                ...
                await database_1.db.execute("DROP TABLE system_logs");
            }
        }
    }
    catch (e) { }
...
exports.ensureSystemLogTable = ensureSystemLogTable;
// ─── Log Rotation ───────────────────────────────────────────────────────────
let lastPruneCheck = 0;
const pruneIfNeeded = async () => {
    const now = Date.now();
    if (now - lastPruneCheck < 60 * 60 * 1000)
        return;
    lastPruneCheck = now;
    try {
        await database_1.db.execute(`DELETE FROM system_logs WHERE created_at < DATE_SUB(NOW(), INTERVAL 90 DAY)`);
        const [countRows] = await database_1.db.execute(`SELECT COUNT(*) as cnt FROM system_logs`);
        const count = countRows[0]?.cnt || 0;
        if (count > 10000) {
            await database_1.db.execute(`DELETE FROM system_logs ORDER BY created_at ASC LIMIT ${count - 10000}`);
        }
    }
    catch (e) {
...
const writeLog = async (payload) => {
    setImmediate(async () => {
        try {
            await (0, exports.ensureSystemLogTable)();
            await pruneIfNeeded();
            const sql = `
            ...
            await database_1.db.execute(sql, params);
        }
        catch (e) {
...
const createLog = async (payload) => {
    await (0, exports.writeLog)(payload);
};
