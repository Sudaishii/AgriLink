...
exports.updatePassword = exports.resetPasswordByToken = exports.forgotPassword = exports.loginUser = exports.registerUser = void 0;
...
const registerUser = async (data) => {
    ...
    try {
        ...
        const [userResult] = await connection.execute('INSERT INTO users_table (auth_id, first_name, last_name, role, city, province) VALUES (?, ?, ?, ?, ?, ?)', [authId, data.firstName || '', data.lastName || '', data.role_name, data.city || '', data.province || '']);
...
const loginUser = async (data) => {
    ...
    const user = users[0];
    // 1. Automatic Resend Verification if not verified
    if (user.is_verified === 0) {
        const verificationToken = jsonwebtoken_1.default.sign({ email: data.email, userId: user.auth_id }, process.env.JWT_SECRET || 'secret', { expiresIn: '15m' });
        const confirmationURL = `http://localhost:${process.env.PORT || 5002}/api/auth/verify?token=${verificationToken}`;
        const logoUrl = process.env.LOGO_URL || 'https://osgogrbrjlnslgdinhgl.supabase.co/storage/v1/object/public/email-assets/AgriLinkGREEN.png';
        const emailHtml = (0, emailTemplates_1.getEmailVerificationTemplate)(confirmationURL, logoUrl);
        console.log('[Auth] Auto-resending verification email to:', data.email);
        (0, emailService_1.sendEmail)({
            to: data.email,
            subject: 'Verify Your AgriLink Account',
            html: emailHtml
        }).catch(err => console.error('[Auth] Failed to auto-resend verification', err));
        throw new Error('Please verify your email address. We have just sent a fresh verification link to your inbox.');
    }
...
const forgotPassword = async (email) => {
    const [users] = await database_1.db.execute('SELECT id, is_verified FROM auth_table WHERE email = ?', [email]);
    if (users.length === 0)
        return; // Silent return for security
    const user = users[0];
    const resetToken = jsonwebtoken_1.default.sign({ email, type: 'reset' }, process.env.JWT_SECRET || 'secret', { expiresIn: '20m' });
    ...
    await (0, emailService_1.sendEmail)({
      ...
      <div style="font-family: sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
        ...
        <p>You requested a password reset for your AgriLink account. Click the button below to set a new password:</p>
        <a href="${resetURL}" style="display: inline-block; padding: 12px 24px; background-color: #5ba409; color: white; text-decoration: none; border-radius: 5px; font-weight: bold;">Reset Password</a>
        <p style="margin-top: 20px; font-size: 12px; color: #999;">This link will expire in 20 minutes. If you did not request this, please ignore this email.</p>
      </div>
    `
    });
};
exports.forgotPassword = forgotPassword;
const resetPasswordByToken = async (token, newPass) => {
    const decoded = jsonwebtoken_1.default.verify(token, process.env.JWT_SECRET || 'secret');
    if (decoded.type !== 'reset')
        throw new Error('Invalid token type');
    const h = await (0, hash_1.hashPassword)(newPass);
    await database_1.db.execute('UPDATE auth_table SET password_hash = ? WHERE email = ?', [h, decoded.email]);
};
