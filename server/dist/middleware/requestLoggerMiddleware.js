...
const deriveModule = (path) => {
    ...
    if (path.startsWith('/api/reviews'))
        return 'Reviews';
    if (path.startsWith('/api/phenotyping'))
        return 'Phenotyping';
    return 'System';
};
const deriveEventType = (method, path) => {
...
const deriveSeverity = (statusCode) => {
    if (statusCode >= 500)
        return 'critical';
    if (statusCode >= 400)
        return 'warning';
...
const deriveCategory = (path, statusCode) => {
    if (path.startsWith('/api/auth'))
        return 'Security';
    if (path.startsWith('/api/users') && statusCode >= 400)
        return 'Security';
    if (path.startsWith('/api/users'))
        return 'User';
    if (path.startsWith('/api/purchases'))
...
const requestLoggerMiddleware = (req, res, next) => {
    ...
    const correlationId = (0, crypto_1.randomBytes)(6).toString('hex');
