...
const mapCartRow = (row: CartRow): CartItemDTO => ({
  id: Number(row.id),
...
export const getCartByUser = async (userId: number): Promise<CartItemDTO[]> => {
...
export const upsertCartItem = async (
...
): Promise<{ item: CartItemDTO; capped: boolean }> => {
  await ensureCartTable();
  ...
  const [existing]: any = await db.execute(
    'SELECT quantity FROM cart_table WHERE user_id = ? AND product_id = ? LIMIT 1',
    [userId, productId]
  );
  ...
  if (nextQty <= 0) {
    await db.execute('DELETE FROM cart_table WHERE user_id = ? AND product_id = ?', [userId, productId]);
    return { item: mapCartRow({ ...product, quantity: 0 }), capped };
  }

  await db.execute(
    `INSERT INTO cart_table (user_id, product_id, quantity)
     VALUES (?, ?, ?)
     ON DUPLICATE KEY UPDATE quantity = VALUES(quantity), updated_at = CURRENT_TIMESTAMP`,
    [userId, productId, nextQty]
  );
...
export const clearCartByUser = async (userId: number): Promise<void> => {
  await ensureCartTable();
  await db.execute('DELETE FROM cart_table WHERE user_id = ?', [userId]);
};
