...
export const ensureFarmerServiceReviewTable = async () => {
    if (ensuredFarmerServiceReviewTable) return;
...
export const reviewService = {
    ...
    addFarmerServiceReview: async (reviewerId: number, reqId: number, rating: number, comment: string) => {
        await ensureFarmerServiceReviewTable();

        const [result]: any = await db.query(
            `INSERT INTO farmer_service_reviews (req_id, reviewer_id, farmer_id, rating, comment, product_id, product_name_snapshot)
             SELECT
                pt.req_id,
                ?,
                p.u_id AS farmer_id,
             ...
             WHERE pt.req_id = ?
               AND pt.buyer_id = ?
               AND pt.req_status = 'Completed'`,
            [reviewerId, rating, comment, reqId, reviewerId]
        );
    ...
    getFarmerServiceReviewsByFarmerId: async (farmerId: number) => {
        await ensureFarmerServiceReviewTable();
    ...
    getFarmerServiceRatingStats: async (farmerId: number) => {
        await ensureFarmerServiceReviewTable();
