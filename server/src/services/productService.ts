...
const ensureStockThresholdColumns = async () => {
    if (ensuredStockThresholdColumns) return;
...
export const getAllProducts = async (brgy?: string) => {
...
export const getLandingSnapshot = async () => {
    await ensureStockThresholdColumns();
    await ensureFarmerServiceReviewTable();
    ...
    const [featuredRows]: any = await db.execute(`
        SELECT p.*, c.cat_name, u.first_name, u.last_name, u.city, u.address, u.farm_address, u.farm_city,
               (SELECT AVG(rating) FROM farmer_service_reviews fsr WHERE fsr.farmer_id = p.u_id) as sellerAvgRating,
               (SELECT COUNT(*) FROM farmer_service_reviews fsr WHERE fsr.farmer_id = p.u_id) as sellerReviewCount,
               EXISTS(SELECT 1 FROM farmer_badges fb WHERE fb.farmer_id = p.u_id AND fb.revoked_at IS NULL AND fb.badge_type = 'verified_farmer') as isVerified
...
export const createProduct = async (productData: any) => {
    ...
    if (!p_image) throw new Error('A product image is required. Please upload an image for your listing.');

    // Verify if farmer has set their farm address
    const [farmCheck]: any = await db.execute(
        `SELECT u.farm_address, f.farm_address as formal_farm_address, u.address as home_address
         FROM users_table u
    ...
    await db.execute(
        `UPDATE product_table
         SET p_name = ?,
             ...
             harvest_date = ?
         WHERE p_id = ?`,
        [p_name, p_description, p_price, p_unit, p_quantity, p_quantity, p_category, p_image, p_status, harvest_date, pId]
    );
};
...
export const archiveProduct = async (pId: number, uId: number) => {
    ...
    const [orders]: any = await db.execute(
        'SELECT req_id FROM purchase_table WHERE product_id = ? AND req_status IN ("Pending", "Confirmed", "Processing")',
        [pId]
    );
