...
import { getImageEmbedding } from '../services/imageEmbeddingService';
...
const run = async () => {
    ...
    for (const row of rows) {
        const imagePath = row.p_image as string;
        ...
        try {
            ...
            const embedding = await getImageEmbedding(buffer, mimeType);
            await db.execute(
                'UPDATE product_table SET p_image_embedding = ? WHERE p_id = ?',
                [JSON.stringify(embedding), row.p_id]
            );
